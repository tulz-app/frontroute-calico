...
private[frontroute] def extractContext: Directive[frontroute.Location] =
  Directive[frontroute.Location](inner => (location, previous, state) => inner(location)(location, previous, state))

private[frontroute] def extract[T](f: frontroute.Location => T): Directive[T] =
